...
"""
Executa a migração `migracao_eventos_agenda.sql` contra o `core_db` — cria a
tabela `eventos_agenda` do novo módulo "Agenda do Conselho" (ver
`claude/decisao-controle-acesso-cadastro.md` no Project "Core", seção 9).
Mesmo padrão já usado em `rodar_migracao_operador_administrativo_loja.py` e
...
ARQUIVO_SQL = os.path.join(os.path.dirname(os.path.abspath(__file__)), "migracao_eventos_agenda.sql")
...
try:
    ...
    print("- Tabela 'eventos_agenda' criada (ou já existia).")
