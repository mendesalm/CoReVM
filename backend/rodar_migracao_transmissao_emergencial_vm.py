...
"""
Executa a migração `migracao_transmissao_emergencial_vm.sql` contra o
`core_db`, sem depender do cliente `psql` — usa `psycopg2` (já é dependência
do projeto) e lê a mesma variável de ambiente `DATABASE_URL_CORE` que o
próprio backend usa em `database.py`. Mesmo padrão já usado em
`rodar_migracao.py` / `rodar_migracao_diretoria_loja_id.py`.

Como rodar (dentro de `CoReVM/backend`, no mesmo venv do projeto):
    python rodar_migracao_transmissao_emergencial_vm.py
...
ARQUIVO_SQL = os.path.join(os.path.dirname(os.path.abspath(__file__)), "migracao_transmissao_emergencial_vm.sql")
...
try:
    ...
    print("- Colunas 'pode_indicar_veneravel' e 'concedido_em' adicionadas em suplentes_conselho.")
